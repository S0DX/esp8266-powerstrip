/**
 * @file    sy7t609_uart.cpp
 * @brief   SY7T609 电能计量芯片 - UART 通信驱动实现
 *
 ...
 *
 * @description
 *   本文件实现 SY7T609_UART 类的全部方法：
 *   - 测量值解析函数（4 种：无符号/绝对值/有符号/原始值，均 /1000 转工程量）
 *   - 表驱动测量值注册表（7 项，新增数据项只需加一行）
 *   - 生命周期：setup(软复位) / loop(状态机+超时恢复) / update(发起轮询)
 *   - SSI 协议读写命令封装
 *   - 动作回调队列（延迟执行，避免与读取状态机冲突）
 *   - 6 种校准动作的具体实现
 *
 * @communication
 *   SSI 协议帧格式：
 *   - 帧头: 0xAA
 *   - 长度: 1 字节（含帧头、长度字节本身、命令、数据、校验和）
 *   - 命令: 0xA3=选寄存器地址, 0xE3=读3字节, 0xD3=写3字节
 *   - 数据: 小端序，3 字节（24bit）
 *   - 校验: 所有字节求和取反加1（二进制补码校验和）
 *
