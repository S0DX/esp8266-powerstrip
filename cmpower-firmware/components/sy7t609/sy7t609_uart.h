/**
 * @file    sy7t609_uart.h
 * @brief   SY7T609 电能计量芯片 - UART 通信驱动类与 ESPHome Action 定义
 *
 ...
 *
 * @description
 *   本文件定义了 SY7T609_UART 类，继承 PollingComponent + UARTDevice：
 *   - 表驱动测量值轮询：7 项测量值（功率因数/电压/电流/有功功率/无功功率/电量/温度）
 *     通过 get_measurements_() 注册表配置，新增数据项只需加一行
 *   - SSI 协议读写：uartSendReadCmd / uartSendWriteCmd
 *   - 6 种 ESPHome Action：reset_energy / reset_calibration / calibrate_voltage
 *     / calibrate_current / calibrate(通用) / save_calibration
 *   - 动作回调队列：Action::play 只入队，实际执行延迟到 loop() 中同步执行
 *     （避免与读取状态机冲突）
 *
 * @design
 *   逻辑开关 vs 真实开关：
 *   - 真实开关直接控制 GPIO 继电器输出
 *   - 逻辑开关面向用户，保证从继电器只有在主继电器接通时才显示"开启"
 *
