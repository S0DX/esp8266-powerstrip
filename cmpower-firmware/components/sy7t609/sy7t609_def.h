/**
 * @file    sy7t609_def.h
 * @brief   SY7T609 电能计量芯片 - 寄存器地址表、协议常量与校准默认值定义
 *
 ...
 *
 * @description
 *   本文件定义了 SY7T609 芯片 SSI 串行协议所需的全部常量：
 *   - 校准/缩放寄存器的出厂固定值（用于 reset_calibration 动作）
 *   - SSI 协议帧头、帧长、命令字
 *   - 寄存器地址映射表（计量寄存器 + 校准寄存器）
 *   - COMMAND 寄存器的命令码（软复位、清零、存Flash、自动校准）
 *   - SSI 回复码枚举
 *   - 内部读取状态机枚举
 *
